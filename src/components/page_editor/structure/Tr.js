...
import { fetchPage, fetchChildPage } from '../../../actions/page_editor/pageActions.js'
...
class Tr extends Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    if (nextProps) {
      if (nextProps.page.children.length !== 0) {
        this.setState({
          isLoading: false
        })
      }
  ...
  handleFirstPageFetch = (id, event) => {
    ...
    // Collapse
    $('.'+id+'.first-level-tr').collapse('toggle')

    // If Tr is not in 'show' state remove class 'show' from all chid trs
    if(!$('.'+id+'.first-level-tr').hasClass('show')) {
      $('tr').removeClass('show')
    }
  }

  handleSecondLevelPageFetch = (id, event) => {
    event.preventDefault()
    event.stopPropagation()
  ...
  handleThirdLevelPage = (id, event) => {
    event.preventDefault()
    event.stopPropagation()

    console.log('handleThirdLevelPage')
  }

  // Handle Input, Select and Checkbox changes.
  handleChange = () => {
  ...
  render() {
    ...
    console.log('Tr children: ', this.props.page.children)
    
    // Array for thirdLevelPages
    let thirdLevelTrs

    const secondLevelTrs = (
      Array.isArray(this.props.page.children) && this.props.page.children.map(page => 

        <Tr  key={page._id} id={this.props.id} page={page} fetchPage={fetchPage} fetchChildPage={fetchChildPage} onClick={this.handleSecondLevelPageFetch}> 

          { thirdLevelTrs = (
            Array.isArray(page.children) && page.children.map(page => 

              <Tr  key={page._id} id={this.props.id} page={page} fetchPage={fetchPage} fetchChildPage={fetchChildPage} onClick={this.handleThirdLevelPage} /> 
            )  
          )}
        </Tr>
      )  
    ...
    return(        
      <tr id={this.props.id ? this.props.id : ""+_id+""} className={classnames(''+_id+'', {'first-level-tr': !_parentId, children: children, spinner: isLoading })} onClick={this.props.onClick ? this.props.onClick.bind(this, _id) : this.handleFirstPageFetch.bind(this, _id)} data-tt-id={""+_id+""} data-tt-parent-id={""+dataTTPrentId+""}>
        ...
        <td colSpan="6" className="p-0">
          <table width="100%">
            <tbody> 
              ...
              <tr className={""+_id+" first-level-tr p-0"}>
                <td colSpan="6" className="p-0">
                  <table width="100%">
                    <tbody> 
                      { secondLevelTrs && secondLevelTrs }
...
export default connect(mapStateToProps, { fetchPage, fetchChildPage })(Tr)
